...
const webAlertControllers = {
  alertsAPI : async (request,response,next) => {
    ...
    const alerts = await WebAlert.find({ fileUpload_id : fileId }).select("_id alert_id alert_type created_at severity rule_id rule_name status").sort({ created_at : 1 });
  ...
  alertAPI : async (request,response,next) => {
    ...
    const alert = await WebAlert.findById(alertId);
    if (!alert) {
  ...
  updateStatusAPI : async (request,response,next) => {
    ...
    const alert = await WebAlert.findById(alertId).populate({path: "fileUpload_id",select: "uploadedBy"});
  ...
  deleteLogAPI : async (request,response,next) => {
    ...
    await WebAlert.deleteMany({ fileUpload_id : file._id });
  ...
  filteredAlertsAPI : async (request,response,next) => {
    ...
    const filteredAlerts = await WebAlert.find(filterDraft).select("_id alert_id alert_type severity status created_at rule_id rule_name");
...
export default webAlertControllers;